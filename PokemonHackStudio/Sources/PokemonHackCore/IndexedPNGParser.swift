...
public struct IndexedTilesetImage: Codable, Equatable, Sendable {
    ...
    public var tileCount: Int {
        max(width / 8, 0) * max(height / 8, 0)
    }

    public func paletteIndex(tileIndex: Int, x: Int, y: Int, hFlip: Bool, vFlip: Bool) -> UInt8? {
...
public enum IndexedPNGParser {
    public static func parse(url: URL) -> IndexedTilesetImage? {
        guard let data = try? Data(contentsOf: url) else { return nil }
        return parse(data: data)
    }

    public static func parse(data: Data) -> IndexedTilesetImage? {
