...
ROM mutation artifact libraries are read-only review surfaces: they may scan existing ignored `.pokemonhackstudio/rom-mutations/**/apply-manifest.json` files and recorded backups under the selected workspace artifact root for manifest/base/backup status, but must not create directories, create dry-run manifests, apply bytes, author replacements, repair checksums, export ROMs, overwrite artifacts, launch emulators, mutate source, or hash files outside that artifact root.
