...
- Evolution add/remove/reorder, trainer class/resource JSON, nested trainer/item/personal/encounter/event arrays or objects outside the explicitly exposed scalar tuple values, nested field-event directories, map matrices/tables, HeartGold/SoulSilver CSV item rows and non-JSON encounter/event rows, other Diamond/Pearl C anchors, Diamond/Pearl non-integer item mapping or trainer class gender expressions, binary item/personal/trainer/map/script data, ROM-backed rows, NARC/container rows, generated/reference rows, PMD-Sky, extraction, rebuilds, binary writes, and ROM exports remain blocked/read-only.
