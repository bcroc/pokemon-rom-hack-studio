...
7. **Done - Semantic Editor Design V1**: eligible Platinum source-backed JSON species, move, item, trainer, and existing species evolution tuple records plus HeartGold/SoulSilver personal JSON records expose scalar semantic fields in Resources and CLI plan/apply commands while still lowering to the source-backed NDS mutation plan. Separate binary-only export plans remain later rows.
