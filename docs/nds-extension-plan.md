...
7. **Done - Semantic Editor Design V1**: eligible Platinum source-backed JSON species, move, item, trainer data, direct trainer-class JSON, direct encounter JSON, direct field-event JSON, direct map-matrix JSON, existing source text lines under `res/text/**/*.txt`, and existing species evolution tuple records, HeartGold/SoulSilver personal/trainer/item JSON, direct item CSV, and nested encounter JSON records, Diamond/Pearl personal/trainer JSON records, Diamond/Pearl `arm9/src/itemtool.c` `sItemIndexMappings` integer initializer rows, and Diamond/Pearl `arm9/src/trainer_data.c` `sTrainerClassGenderCountTbl` integer initializer rows expose scalar semantic fields in Resources and CLI plan/apply commands while still lowering to the source-backed NDS mutation plan. Separate binary-only export plans remain later rows.
...
- `NDSDataSemanticEditor` adds the first semantic layer above raw source text by detecting top-level scalar fields in eligible Platinum source-backed JSON records for species/personal/move-style data, item data under `res/items/**/*.json`, trainer data under `res/trainers/data/**/*.json`, direct trainer-class JSON rows under `res/trainers/classes/*.json`, direct encounter JSON rows under `res/field/encounters/*.json`, direct field-event JSON rows under `res/field/events/*.json`, direct map-matrix JSON rows under `res/field/matrices/*.json`, existing source text lines under `res/text/**/*.txt`, existing species evolution tuple method/parameter/target values under `res/pokemon/<species>/data.json`, HeartGold/SoulSilver personal/trainer/item JSON rows under `files/poketool/personal/**/*.json`, `files/poketool/trainer/**/*.json`, and `files/itemtool/itemdata/**/*.json`, direct HeartGold/SoulSilver item CSV rows under `files/itemtool/itemdata/*.csv`, nested HeartGold/SoulSilver encounter JSON rows under `files/fielddata/encountdata/**/*.json`, direct HeartGold/SoulSilver zone-event JSON rows under `files/fielddata/eventdata/zone_event/*.json`, Diamond/Pearl personal JSON rows under `files/poketool/personal/**/*.json` and `files/poketool/personal_pearl/**/*.json`, Diamond/Pearl trainer/item/encounter JSON rows under `files/poketool/trainer/**/*.json`, `files/itemtool/itemdata/**/*.json`, and `files/fielddata/encountdata/**/*.json`, direct Diamond/Pearl field-event JSON rows under `files/fielddata/eventdata/*.json`, Diamond/Pearl `arm9/src/itemtool.c` `sItemIndexMappings` integer initializer scalars, and Diamond/Pearl `arm9/src/trainer_data.c` `sTrainerClassGenderCountTbl` integer initializer scalars.
...
- Evolution add/remove/reorder, `res/trainers/resources/**`, nested trainer-class directories, nested trainer/item/personal/encounter/event arrays or objects outside the explicitly exposed scalar tuple values, nested field-event directories, nested map-matrix directories, map tables, `res/field/maps/**`, `res/field/area_data/**`, non-`.txt` Platinum text rows, BMG/message-bank writers, multiline/insert/delete/reorder text edits, nested HeartGold/SoulSilver CSV item rows, HGSS non-CSV/binary item rows, non-JSON encounter/event rows, HGSS/DP trainer-class equivalents, other Diamond/Pearl C anchors, Diamond/Pearl non-integer item mapping or trainer class gender expressions, binary item/personal/trainer/map/script data, ROM-backed rows, NARC/container rows, generated/reference rows, PMD-Sky, extraction, NARC/container rebuild or repack, build/playtest execution, binary writes, ROM rebuild, and ROM exports remain blocked/read-only.
...
Expand semantic Gen IV coverage one dedicated source-backed domain at a time, continuing from the completed Platinum rows including direct trainer-class JSON, direct map-matrix JSON, and source text-line fields, HGSS personal/trainer/item/encounter/zone-event JSON slices, HGSS direct item CSV cells, Diamond/Pearl personal/trainer/item/encounter/field-event JSON slices, and the first Diamond/Pearl item/trainer C-anchor slices toward other HGSS or Diamond/Pearl source-backed schemas only when they fit the existing mutation-plan architecture. Keep container/ROM writes disabled until dedicated parser, preservation, and rebuild rows exist, and keep NDS build/playtest as manual guidance until a future implementation row opens runnable execution. Future Gen V work remains preview-only source inventory until dedicated parser/planner/apply rows explicitly land.
