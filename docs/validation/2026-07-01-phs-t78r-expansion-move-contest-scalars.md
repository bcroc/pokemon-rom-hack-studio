...
- `swift test --package-path PokemonHackStudio --scratch-path /tmp/phs-t78q-swiftpm --filter 'PokemonMoveCatalogTests|PokemonDataCompatibilityTests'` (blocked by unrelated all-learnables compatibility assertions after the PHS-T78R move catalog and compatibility tests passed; failures remain in `testExpansionAllLearnablesCoverageCountsGeneratedSourceAndMoveMismatches` and `testExpansionLearnsetCompatibilityWarnsWhenAllLearnablesIsStale`.)
- `make validate` (blocked by the same unrelated all-learnables compatibility assertions after the SwiftPM suite ran; PHS-T78R core, compatibility, and CLI JSON tests passed inside the run.)
