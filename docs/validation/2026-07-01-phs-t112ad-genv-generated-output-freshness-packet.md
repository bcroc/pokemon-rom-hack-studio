...
- `POKEMONHACKSTUDIO_SKIP_BUNDLE_ASSETS=1 xcodebuild -quiet -project PokemonHackStudio/PokemonHackStudio.xcodeproj -scheme PokemonHackStudio -configuration Debug -derivedDataPath /tmp/PokemonHackStudio-PHS-T112AD-App -destination 'platform=macOS,arch=arm64' -parallel-testing-enabled NO -only-testing:PokemonHackStudioTests/MapEditorStoreTests/testGenVGeneratedOutputFreshnessPacketStaysPreviewOnlyInResourcesSelection test` was blocked before the selected test by existing app compile drift in `PokemonHackStudio/Sources/PokemonHackStudio/Views/Editors/BuildWorkbenchView.swift:685:13`: `switch must be exhaustive`, missing `.mapRenderAudit`.
