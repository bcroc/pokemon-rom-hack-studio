...
No implementation row is currently active. The completed baseline now includes `PHS-T54`, `PHS-T55`, `PHS-T57B`, `PHS-T61`, and `PHS-T62`: graphics metatile/palette source authoring drafts, map workflow planning/prefab paste polish, FireRed item row-field editing, persistent ignored resource-index cache plus app-side Resources row indexes, and context-aware app/Maps toolbar commands. Source writes remain behind preview-first mutation gates where they exist, and resource caches stay local artifacts that rebuild from source on stale metadata.
