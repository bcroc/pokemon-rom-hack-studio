...
No implementation row is currently active. The completed baseline now includes `PHS-T49` through `PHS-T53` plus `PHS-T58`: read-only Pokemon data compatibility reporting, Graphics import package detail preview UI, script/text authoring helpers through map-local mutation gates, non-mutating patch artifact and binary ROM diff/repoint planning, and lazy/cancellable startup plus project-switching loads. Follow-up reference rows remain Candidate until the compatibility matrix, preview surfaces, and runtime loading proof drive a narrower editor lane.
...
| PHS-T54 | Candidate | Graphics Authoring Mutation Plans | Follow `PHS-T50` with metatile and palette edit mutation plans after package provenance and conversion-readiness display is proven. |
| PHS-T55 | Candidate | Map Workflow Polish | Add map/layout duplication planning, file-watch reload prompts, region-map preview, prefab/paste flows, and visual export planning without weakening source-write gates. |
...
| PHS-T57 | Candidate | Compatibility-Specific Data Editors | Continue splitting TM/HM/tutor editing, Ruby/Sapphire data edits, Expansion/Modern Emerald support, and remaining compatibility-specific row-field editors into rows driven by the `PHS-T49` compatibility matrix. |
