...
No implementation row is currently active. The live baseline includes `PHS-T68`, `PHS-T69`, `PHS-T70`, `PHS-T71`, `PHS-T72`, `PHS-T74`, `PHS-T80`, `PHS-T81`, `PHS-T82`, `PHS-T86`, `PHS-T87`, `PHS-T88`, `PHS-T89`, `PHS-T90`, `PHS-T91`, `PHS-T91B`, `PHS-T91C`, `PHS-T91D`, `PHS-T95`, `PHS-T96`, and `PHS-T98A`; choose new work from the remaining Active Board candidate rows.
...
| PHS-T94 | Candidate | NDS Graphics Container Preview V1 | Add read-only NDS graphics/container facts and tiny preview metadata for formats such as NCLR, NCGR, NSCR, NCER, NANR, NFTR, BMG, NSBMD, and NSBTX while keeping extraction, conversion, rebuild, mutation apply, and ROM writes disabled. |
...
| PHS-T98A | Done | Platinum Trainer Semantic Fields V1 | Eligible Platinum `res/trainers/data/**/*.json` rows now expose top-level scalar semantic fields and apply through the existing NDS raw-source mutation gate; trainer class/resource JSON, nested arrays/objects, HGSS/DP, PMD-Sky, NARC/container, generated/reference, ROM, extraction, rebuild, and binary-write paths remain blocked. |
...
| PHS-T100 | Candidate | NDS Container Member Fingerprints | Add read-only magic/extension/size/compression confidence hints for NARC and unpacked archive members so graphics, text, map, and migration rows can route opaque members without decompression writes. |
...
- `PHS-T95`:
  - `swift test --package-path PokemonHackStudio --filter NDSDataCatalogTests` (14 selected tests; readiness, related rows, trainer semantic coverage, NDS catalog, and resource registry coverage passed)
  - `swift test --package-path PokemonHackStudio --filter 'NDSDataCatalog|NDS|PokemonHackCLI'` (44 selected tests; NDS catalog/ROM/decomp/toolchain/CLI coverage passed)
  - `cd PokemonHackStudio && xcodebuild -quiet -project PokemonHackStudio.xcodeproj -scheme PokemonHackStudio -configuration Debug -derivedDataPath ../DerivedData/PokemonHackStudio-PHS-T95 -destination 'platform=macOS,arch=arm64' -only-testing:PokemonHackStudioTests/MapEditorStoreTests/testNDSSemanticJSONFieldEditsFlowThroughResourceEditor test` (passed; Resources rendered NDS readiness/related-domain facts and semantic field edits still flowed through the existing mutation gate)
  - Central reference smoke attempt: full `nds-data-catalog` runs against `pret__pokeplatinum`, `pret__pokeheartgold`, and `pret__pokediamond` were stopped after several minutes with no output because the full reference clone scans were too broad for an interactive proof; synthetic NDS fixtures and focused catalog/CLI tests covered the changed code path.
  - `make test` (231 SwiftPM tests; passed)
  - `make validate` (passed; includes 231 SwiftPM tests and the existing CLI smoke ladder)
  - `git diff --check` (passed)
  - Source-write posture: PHS-T95 is read-only metadata integration only; it adds readiness summaries, related record IDs, facts, and diagnostics, but no editor, compiler, extraction, rebuild, mutation apply, NARC packing, ROM export, or binary/container write behavior.
- `PHS-T82`:
