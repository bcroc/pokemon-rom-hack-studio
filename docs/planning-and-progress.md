...
No implementation row is currently active. The completed baseline now includes `PHS-T49` through `PHS-T52` plus `PHS-T58`: read-only Pokemon data compatibility reporting, Graphics import package detail preview UI, script/text authoring helpers through map-local mutation gates, non-mutating patch artifact planning, and lazy/cancellable startup plus project-switching loads. Follow-up reference rows remain Candidate until the compatibility matrix, preview surfaces, and runtime loading proof drive a narrower editor lane.
...
| PHS-T53 | Candidate | Binary ROM Diff And Repoint Planning | Follow `PHS-T52` with read-only binary diff previews, free-space suitability, pointer-repoint plans, backup/export manifests, and blocked apply state for ROM-only workflows. |
...
| PHS-T56 | Candidate | Playtest Capture Artifacts | Add explicit mGBA screenshot and savestate capture actions as separate artifact-producing flows from ordinary launch. |
| PHS-T57 | Candidate | Compatibility-Specific Data Editors | Split TM/HM/tutor editing, item description text editing, Ruby/Sapphire data edits, and Expansion/Modern Emerald support into rows driven by the `PHS-T49` compatibility matrix. |
